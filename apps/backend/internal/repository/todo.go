...
import (
	...
	"github.com/fakelozic/tasker/internal/errs"
	"github.com/fakelozic/tasker/internal/model"
	"github.com/fakelozic/tasker/internal/model/todo"
...
func (r *TodoRepository) CreateTodo(ctx context.Context, userID string, payload *todo.CreateTodoPayload) (*todo.Todo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to to collect row from table:todos for user_id=%s title=%s: %w", userID, payload.Title, err)
	}
...
`
	...
	if err != nil {
		return nil, fmt.Errorf("failed to execute collect row from table:todos for todo_id=%s user_id%s: %w", todoID.String(), userID, err)
	}
...
func (r *TodoRepository) UpdatedTodo(ctx context.Context, userID string, payload *todo.UpdateTodoPayload) (*todo.Todo, error) {
...
func (r *TodoRepository) UploadTodoAttachment(ctx context.Context, todoID uuid.UUID, userID string, s3Key string, fileName string, fileSize int64, mimeType string) (*todo.TodoAttachment, error) {
	...
	attachment, err := pgx.CollectOneRow(rows, pgx.RowToAddrOfStructByName[todo.TodoAttachment])
	if err != nil {
		return nil, fmt.Errorf("failed to collect row form table:todo_attachments: %w", err)
	}
...
func (r *TodoRepository) GetTodosDueInHours(ctx context.Context, hours int, limit int) ([]todo.Todo, error) {
	stmt := `
		...
		WHERE
			...
			AND due_date > NOW()
			AND due_date <= NOW() + INTERVALE '%d hours'
			AND stats NOT IN ('completed', 'archived')
		ORDER BY
			due_date ASC
		LIMIT
			%d
...
func (r *TodoRepository) GetOverdueTodos(ctx context.Context, limit int) ([]todo.Todo, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to collect rows from the table:todos: %w", err)
	}
...
func (r *TodoRepository) GetWeeklyStatsForUsers(ctx context.Context, startDate, endDate time.Time) ([]todo.UserWeeklyStats, error) {
	stmt := `
		SELECT
			...
			COUNT(*) FILTER (WHERE status = 'completed' AND completed_at >= @start_date AND competed_at <= @end_date) AS completed_count,
			COUNT(*) FILTER (WHERE status NOT IN ('completed', 'archive')) AS active_count,
			COUNT(*) FILTER (WHERE dude_date < NOW() AND status NOT IN ('completed', 'archived')) AS overdue_count
		FROM
			todos
		GROUP BY
			user_id
	...
	stmt := `
		SELECT
			...
			COALESCE(
				jsonb_agg(
					CASE
						WHEN child.id IS NOT NULL to_jsonb(camel(child))
						ELSE NULL
					END
				) FILTER (
				 	WHERE child.id IS NOT NULL
