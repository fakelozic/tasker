...
import (
	...
	testing_pkg "github.com/fakelozic/tasker/internal/testing"
...
func TestTodoRepository_GetTodoByID(t *testing.T) {
	_, testSever, cleanup := testing_pkg.SetupTest(t)
	defer cleanup()

	ctx := context.Background()
	todoRepo := repository.NewTodoRepository(testSever)
...
func TestTodoRepository_GetTodos(t *testing.T) {
	...
	t.Run("filter by status", func(t *testing.T) {
		...
		t.Run("search by title", func(t *testing.T) {
			...
			for _, todoItem := range result.Data {
				assert.Contains(t, todoItem.Title, &search)
			}
		})
...
func TestTodoRepository_UpdateTodo(t *testing.T) {
	...
	t.Run("update todo title successfully", func(t *testing.T) {
		newTitle := "Update Todo Title"
		payload := &todo.UpdateTodoPayload{
			ID:    testTodo.ID,
			Title: &newTitle,
		}

		result, err := todoRepo.UpdatedTodo(ctx, userID, payload)
		require.NoError(t, err)
		require.NotNil(t, result)
	...
	t.Run("update with no fields should fail", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.Nil(t, result)
		assert.Contains(t, err.Error(), "no field to update")
	})
	...
	t.Run("with canceled context", func(t *testing.T) {
		...
		result, err := todoRepo.UpdatedTodo(canceledCtx, userID, payload)
		assert.Error(t, err)
		assert.Nil(t, result)
	})
}
...
func TestTodoRepository_GetTodoStats(t *testing.T) {
	...
	t.Run("get todo status successfully", func(t *testing.T) {
