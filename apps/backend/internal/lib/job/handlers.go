...
import (
	...
	"github.com/fakelozic/tasker/internal/config"
	"github.com/fakelozic/tasker/internal/lib/email"
...
func (j *JobService) handleReminderEmailTask(ctx context.Context, t *asynq.Task) error {
	...
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
		return fmt.Errorf("failed to unmarshal reminder emil payload: %w", err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("failed to resolve user email fo user %s: %w", p.UserID, err)
	}
...
func (j *JobService) handleWeeklyReportEmailTask(ctx context.Context, t *asynq.Task) error {
	...
	if err != nil {
		j.logger.Error().
			...
			Err(err).
			Msg("Failed to resolve user email")
		return fmt.Errorf("failed to resolve user email to user %s: %w", p.UserID, err)
	}
	...
	if err != nil {
		j.logger.Error().Str("type", "weekly_report").Str("user_id", p.UserID).Err(err).Msg("Failed to send weekly report email")
		return err
	}

	j.logger.Info().Str("type", "weekly_report").Str("user_id", p.UserID).Msg("Successfully sent weekly report email")
	return nil
}
