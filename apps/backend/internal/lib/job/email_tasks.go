...
func EnqueueWeeklyReportEmail(client *asynq.Client, task *WeeklyReportEmailTask) error {
	...
	asynqTask := asynq.NewTask(TaskWeeklyReportEmail, payload, asynq.MaxRetry(3), asynq.Queue("default"), asynq.Timeout(60*time.Second)) // Longer timeout for report generation
