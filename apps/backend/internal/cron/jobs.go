...
func (j *DueDateReminderJob) Description() string {
	return "Enqueue email reminders gor todo due soon"
}
...
func (j *DueDateReminderJob) Run(ctx context.Context, jobCtx *JobContext) error {
	...
	jobCtx.Server.Logger.Info().Int("enqueued_count", enqueuedCount).Int("total_todos", len(todos)).Msg("Due date reminder emails enqueued")

	for userID, titles := range userTodos {
		jobCtx.Server.Logger.Info().Str("user_id", userID).Int("reminder_count", len(titles)).Msg("User reminders enqueued")
	}
...
func (j *OverdueNotificationsJob) Run(ctx context.Context, jobCtx *JobContext) error {
	...
	jobCtx.Server.Logger.Info().Int("todo_count", len(todos)).Msg("Found overdue todos")
	...
	for _, todo := range todos {
		...
		if err != nil {
			jobCtx.Server.Logger.Error().Err(err).Str("todo_id", todo.ID.String()).Str("user_id", todo.UserID).Msg("Failed to enqueue overdue notification")
			continue
		}

		enqueuedCount++
		jobCtx.Server.Logger.Info().Str("todo_id", todo.ID.String()).Str("todo_title", todo.Title).Str("user_id", todo.UserID).Msg("enqueued overdue notification")
	}

	jobCtx.Server.Logger.Info().Int("enqueued_count", enqueuedCount).Int("total_todos", len(todos)).Msg("Overdue notifications enqueued")
...
func (j *WeeklyReportsJob) Run(ctx context.Context, jobCtx *JobContext) error {
	...
	jobCtx.Server.Logger.Info().Int("user_count", len(stats)).Msg("Generating weekly reports")
	...
	for _, userStats := range stats {
		...
		if err != nil {
			jobCtx.Server.Logger.Error().Err(err).Str("user_id", userStats.UserID).Msg("Failed to fetch completed todos")
