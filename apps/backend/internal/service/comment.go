...
import (
	...
	"github.com/fakelozic/tasker/internal/model/comment"
...
func (s *CommentService) UpdateComment(ctx echo.Context, userID string, commentID uuid.UUID, content string) (*comment.Comment, error) {
	...
	_, err := s.commentRepo.GetCommentsByID(ctx.Request().Context(), userID, commentID)
	if err != nil {
	...
	if err != nil {
		logger.Error().Err(err).Msg("failed to updated comment")
		return nil, err
	}
